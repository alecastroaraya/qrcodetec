...
export default function Home() {
  ...
  const [qrs, setqrs] = useState<Array<string>>([]);
  type Props = {
    qrCodes: [QRCode];
  };
  ...
  const handleUpdate = async (QR_Id: string) =>{
    if (name ) {
      try {
        let response = await fetch(`/api/updateQR?id=${QR_Id}`, {
          method: "POST",
          body: JSON.stringify({
            name,
            description,
        ...
        let qrURL = await generateQRCodeDataURL(url);
        setImgSrc(qrURL);
        setMessage("Actualizado datos con éxito");
      ...
      return setError("Necesita el nombre del QR para poder actualizarlo");
    }
  };
      ...
      <div className="ui container border-1">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap' }}>
          ...
          <Form.Field style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', paddingLeft: '50px' }}>
            ...
            <Button onClick={getServerSideProps} style={{ marginLeft: '50px', backgroundColor: '#01899c', color: 'white' }}>Editar</Button>
          </div>
          {imgSrc && (
